...
We call contextual representations \emph{temporally traceable} when
representations of the same word conditioned on requested periods are directly
comparable and changes in their nearest-neighbor composition reflect changes in
...
requires directly comparable representations of uses of \emph{gay} conditioned
on 1920 and 1980.

In this study, we test temporal traceability in a controlled environment where
the usage trajectory is predefined. In natural corpora, an apparent change may instead
reflect differences in word frequency, genre, or corpus composition. It is
therefore difficult to tell whether a change in the learned representations
comes from the model architecture or the data. We introduce a corpus in which
subjects follow predefined stable, drifting, or bifurcating usage trajectories.
Four diagnostics distinguish changes in neighborhood composition, recovery of
the generating group, sensitivity to the period, and transfer to later periods.
...
position. Token-Time instead projects each token--period pair, and
Memory-Augmented also summarizes earlier periods. We ask whether these designs
improve temporal traceability and what each contributes.
...
double the change for drifting subjects; Additive and Token-Time are equivalent
with reliable cues, while Token-Time performs better when they are
uninformative. (4) Memory ablations show that label-derived history improves
later-period classification, whereas representation-derived means and two
\(k\)-means centers do not surpass Token-Time.
...
score comparing two spaces rather than period-conditioned contextual
representations from one model. Standard benchmarks formalize this
...
Our controlled corpus makes each usage trajectory known, allowing differences to
...
period, but do not test whether the neighborhoods of period-conditioned word
representations can be inspected and compared directly.
...
trajectories test whether one mean summary per subject and period is adequate,
and the memory ablations examine alternatives.
...
Each verb and object is drawn from the group opposite to the sentence's
generating group 25\% of the time (Figure~\ref{fig:benchmark-design}, left).
The accompanying words are therefore informative but imperfect cues; subject
identity and period provide the prior \(P(N_1\mid s,t)\), while the generating
group of each sentence remains stochastic. The \texttt{true\_context} field
records this generating group. It is not used in MLM training and is available
only to the diagnostics.
...
known, we can test whether the composition of representation neighborhoods
follows the process that generated the corpus. The scope of these controlled diagnostics is revisited in
the Discussion.

The experiments use four evaluation settings. The standard and ambiguous cue
conditions vary the reliability of the accompanying words. In the standard cue
...
ambiguous cue condition, this probability is reduced to 50\%, so the verb and
object do not reveal which group generated the sentence on average. The
contrastive set presents the same masked-verb subject--object frame with
two period identifiers and measures whether changing only the period changes the
preferred generating group. The later-period transfer set contains periods \(t8\)--\(t9\),
...
condition still reveals a classifier difference.
...
A natural-corpus study can test whether Additive and Token-Time remain
equivalent in neighborhood composition when local contexts identify the current
sense and whether Token-Time gains an advantage for more ambiguous usages. Such
a study must adapt the diagnostics to
